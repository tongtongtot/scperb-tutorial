...
train_path = f"../data/train_{opt.dataset}.h5ad"
valid_path = f"../data/valid_{opt.dataset}.h5ad"
...
# =============================== data gathering ====================================
train_real = data
train_real_stim = train_real[train_real.obs["condition"] == opt.stim_key]
train_real_ctrl = train_real[train_real.obs["condition"] == opt.ctrl_key]
train_real_stim = adata2numpy(train_real_stim)
...
train_real_ctrl = adata2numpy(train_real_ctrl)
...
def train(n_epochs, initial_run=True):
    ...
    for it in range(n_epochs):
        ...
        _, g_loss, d_loss = sess.run([update_G, gen_loss, disc_loss],
                                     feed_dict={X_ctrl: mb_ctrl, X_stim: mb_stim, is_training: True})
        print(f"Iteration {it}: g_loss: {g_loss}  d_loss: {d_loss}")
...
def get_prediction(name, data_name):
    t_in = names[data_name]
    dr = data_reader(opt, data, validation, {"ctrl": opt.ctrl_key, "stim": opt.stim_key}, t_in)
    adata_list = dr.extractor(validation, name)
    ctrl_data = adata_list[1]
    # if sys.argv[1] == "train":
    predicted_cells = predict(adata2numpy(ctrl_data))
    all_Data = sc.AnnData(np.concatenate([adata2numpy(adata_list[1]), adata2numpy(adata_list[2]), predicted_cells]))
    all_Data.obs["condition"] = ["control"] * len(adata2numpy(adata_list[1])) + ["stimulated"] * len(adata2numpy(adata_list[2])) + \
                                ["pred"] * len(predicted_cells)
    all_Data.var_names = adata_list[3].var_names
    os.makedirs(f"../data/reconstructed/STGAN/{opt.dataset}", exist_ok = True)
    all_Data.write(f"../data/reconstructed/STGAN/{opt.dataset}/STGAN_{name}.h5ad")
    print("saved at:" + f"../data/reconstructed/STGAN/{opt.dataset}/STGAN_{name}.h5ad")

def pred_cells(data_name):
    # tf.reset_default_graph()
    # t_in = t_out = names[data_name]
    train(10000, initial_run=True)
    for cell_type in names[data_name]:
        # tf.reset_default_graph()
        # t_out = []
        # data_gather(cell_type)
        # train(1000, initial_run=True)
        get_prediction(cell_type, data_name)
        print(f"finish predicting: {cell_type}")

if __name__ == "__main__":
    print(f"running {opt.dataset}")
    pred_cells(opt.dataset)