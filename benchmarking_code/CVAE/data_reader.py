...
class data_reader():
    ...
    def __init__(self, opt, train_data, valid_data, conditions, tr_ct_list =None,ho_ct_list=None):
        ...
        if(tr_ct_list and ho_ct_list):
            ...
            print(valid_data)
            self.valid_real = self.adata2numpy(valid_data)
    ...
    def extractor(self, data, cell_type):
        cell_with_both_condition = data[data.obs[self.opt.cell_type_key] == cell_type]
        condtion_1 = data[(data.obs[self.opt.cell_type_key] == cell_type) & (data.obs["condition"] == self.conditions["ctrl"])]
        condtion_2 = data[(data.obs[self.opt.cell_type_key] == cell_type) & (data.obs["condition"] == self.conditions["stim"])]
        training = data[~((data.obs[self.opt.cell_type_key] ==cell_type) & (data.obs["condition"] ==self.conditions["stim"]))]
    ...
    def training_data_provider(self,train_s, train_t):
        ...
        train_s_groups = []
        for i in train_s:
            train_s_X.append(self.adata2numpy(i))
            train_s_diet.append(i.obs["condition"].tolist())
            train_s_groups.append(i.obs[self.opt.cell_type_key].tolist())
        ...
        train_t_groups = []
        for i in train_t:
            train_t_X.append(self.adata2numpy(i))
            train_t_diet.append(i.obs["condition"].tolist())
            train_t_groups.append(i.obs[self.opt.cell_type_key].tolist())
        temp = []
        ...
        train_real.obs[self.opt.cell_type_key] = train_s_groups + train_t_groups
        return train_real

    def balancer(self,data):
        class_names = np.unique(data.obs[self.opt.cell_type_key])
        class_pop = {}
        for cls in class_names:
            class_pop[cls] = len(data[data.obs[self.opt.cell_type_key] == cls])
        ...
        for cls in class_names:
            temp = data[data.obs[self.opt.cell_type_key] == cls]
        ...
        balanced_data.obs[self.opt.cell_type_key] = np.concatenate(all_data_label)
        balanced_data.obs["condition"] = np.concatenate(all_data_label)

        class_names = np.unique(balanced_data.obs[self.opt.cell_type_key])
        class_pop = {}
        for cls in class_names:
            class_pop[cls] = len(balanced_data[balanced_data.obs[self.opt.cell_type_key] == cls])
        #print(class_pop)
